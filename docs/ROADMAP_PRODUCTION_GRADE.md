...
| D3D12 backend | 🟡 In progress | `SynapseD3D12Helper.dll` builds and exports real `install_hook`/`remove_hook` APIs; helper-DLL tests pass including real API stress test and auto-attach from Vulkan layer init; COM vtable scaffolding builds but real hook validation remains deferred due to in-process MSVC calling-convention instability |
